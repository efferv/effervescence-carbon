...
pthread_once_t              EFF_Device::sStaticInitializer  = PTHREAD_ONCE_INIT;
EFF_Device*                 EFF_Device::sInstance           = nullptr;
EFF_Device*                 EFF_Device::sUISoundsInstance   = nullptr;

EFF_Device&    EFF_Device::GetInstance()
{
    pthread_once(&sStaticInitializer, StaticInitializer);
    return *sInstance;
}

EFF_Device&    EFF_Device::GetUISoundsInstance()
{
    pthread_once(&sStaticInitializer, StaticInitializer);
    return *sUISoundsInstance;
}

void    EFF_Device::StaticInitializer()
{
    try
    {
        // The main instance, usually referred to in the code as "EFFDevice". This is the device
        // that appears in System Preferences as "Background Music".
        sInstance = new EFF_Device(kObjectID_Device,
                                   CFSTR(kDeviceName),
                                   CFSTR(kEFFDeviceUID),
                                   CFSTR(kEFFDeviceModelUID),
        ...
        sUISoundsInstance = new EFF_Device(kObjectID_Device_UI_Sounds,
                                           CFSTR(kDeviceName_UISounds),
                                           CFSTR(kEFFDeviceUID_UISounds),
                                           CFSTR(kEFFDeviceModelUID_UISounds),
        ...
        EFF_VolumeControl& theUISoundsVolumeControl = sUISoundsInstance->mVolumeControl;
        ...
        // Apply the volume to the device's output stream. The main instance of EFF_Device doesn't
        // apply volume to its audio because EFFApp changes the real output device's volume directly instead.
    ...
    {
        DebugMsg("EFF_Device::StaticInitializer: failed to create the devices");
        delete sInstance;
        sInstance = nullptr;
...
EFF_Device::EFF_Device(AudioObjectID inObjectID,
...
:
    EFF_AbstractDevice(inObjectID, kAudioObjectPlugInObject),
...
{
    // Initialises the loopback clock with the default sample rate and, if there is one,
    // sets the wrapped device to the same sample rate
...
void    EFF_Device::InitLoopback()
{
    ...
    mLoopbackRingBuffer.Allocate(1,
                                 2 * sizeof(Float32),
                                 kLoopbackRingBufferFrameSize);
}


#pragma mark Property Operations
// Basically forwards all (inObjectID == mObjectID) calls to Device_ methods
// and others to owned objects

bool    EFF_Device::HasProperty(AudioObjectID inObjectID,
                                pid_t inClientPID,
...
bool    EFF_Device::IsPropertySettable(AudioObjectID inObjectID,
                                       pid_t inClientPID,
                                       const AudioObjectPropertyAddress& inAddress)
const
{
...
UInt32    EFF_Device::GetPropertyDataSize(AudioObjectID inObjectID,
                                          pid_t inClientPID,
                                          const AudioObjectPropertyAddress& inAddress,
...
void    EFF_Device::GetPropertyData(AudioObjectID inObjectID, pid_t inClientPID,
                                    const AudioObjectPropertyAddress& inAddress,
                                    UInt32 inQualifierDataSize,
                                    const void* inQualifierData,
                                    UInt32 inDataSize,
...
const
{
    ThrowIfNULL(outData, std::runtime_error("!outData"), "EFF_Device::GetPropertyData: !outData");
...
void    EFF_Device::SetPropertyData(AudioObjectID inObjectID,
                                    pid_t inClientPID,
                                    const AudioObjectPropertyAddress& inAddress,
                                    UInt32 inQualifierDataSize,
                                    const void* inQualifierData,
                                    ...
                                    const void* inData)
{
    ThrowIfNULL(inData, std::runtime_error("no data"), "EFF_Device::SetPropertyData: no data");
...
bool    EFF_Device::Device_HasProperty(AudioObjectID inObjectID,
                                       pid_t inClientPID,
                                       const AudioObjectPropertyAddress& inAddress)
const
{
    ...
    switch(inAddress.mSelector)
    {
        ...
        case kAudioDeviceCustomPropertyDeviceIsRunningSomewhereOtherThanEFFApp:
...
bool    EFF_Device::Device_IsPropertySettable(AudioObjectID inObjectID,
                                              pid_t inClientPID,
                                              const AudioObjectPropertyAddress& inAddress)
const
{
...
UInt32    EFF_Device::Device_GetPropertyDataSize(AudioObjectID inObjectID,
                                                 pid_t inClientPID,
                                                 const AudioObjectPropertyAddress& inAddress,
                                                 UInt32 inQualifierDataSize,
                                                 const void* inQualifierData)
...
void    EFF_Device::Device_GetPropertyData(AudioObjectID inObjectID,
                                           pid_t inClientPID,
                                           const AudioObjectPropertyAddress& inAddress,
                                           UInt32 inQualifierDataSize,
                                           const void* inQualifierData,
    ...
    switch(inAddress.mSelector)
    {
        case kAudioObjectPropertyName:
            ...
            ThrowIf(inDataSize < sizeof(AudioObjectID),
                    ...
                    "EFF_Device::Device_GetPropertyData: not enough space for the return value of kAudioObjectPropertyName for the device");
        ...
        case kAudioObjectPropertyManufacturer:
            ...
            ThrowIf(inDataSize < sizeof(AudioObjectID),
                    CAException(kAudioHardwareBadPropertySizeError),
                    "EFF_Device::Device_GetPropertyData: not enough space for the return value of kAudioObjectPropertyManufacturer for the device");
        ...
        case kAudioDevicePropertyDeviceUID:
            ...
            ThrowIf(inDataSize < sizeof(AudioObjectID),
                    CAException(kAudioHardwareBadPropertySizeError),
                    "EFF_Device::Device_GetPropertyData: not enough space for the return value of kAudioDevicePropertyDeviceUID for the device");
        ...
        case kAudioDevicePropertyModelUID:
            ...
            ThrowIf(inDataSize < sizeof(AudioObjectID),
                    CAException(kAudioHardwareBadPropertySizeError),
                    "EFF_Device::Device_GetPropertyData: not enough space for the return value of kAudioDevicePropertyModelUID for the device");
        ...
        case kAudioDevicePropertyDeviceIsRunning:
            ...
            ThrowIf(inDataSize < sizeof(UInt32),
                    CAException(kAudioHardwareBadPropertySizeError),
                    "EFF_Device::Device_GetPropertyData: not enough space for the return value of kAudioDevicePropertyDeviceIsRunning for the device");
            *reinterpret_cast<UInt32*>(outData) = mClients.ClientsRunningIO() ? 1 : 0;
            outDataSize = sizeof(UInt32);
            break;

        case kAudioDevicePropertyDeviceCanBeDefaultDevice:
            // See EFF_AbstractDevice::GetPropertyData.
            //
            // We don't allow the UI Sounds instance of EFF_Device to be set as the default device
            ...
            //
            // Instead, EFFApp sets the UI Sounds device as the "system default" (see
            // kAudioDevicePropertyDeviceCanBeDefaultSystemDevice) so apps will use it for
            // UI-related sounds.
            ThrowIf(inDataSize < sizeof(UInt32),
                    CAException(kAudioHardwareBadPropertySizeError),
                    "EFF_Device::GetPropertyData: not enough space for the return value of "
                    "kAudioDevicePropertyDeviceCanBeDefaultDevice for the device");
            // TODO: Add a field for this and set it in EFF_Device::StaticInitializer so we don't
        ...
        case kAudioDevicePropertyNominalSampleRate:
            ...
            ThrowIf(inDataSize < sizeof(Float64),
                    CAException(kAudioHardwareBadPropertySizeError),
                    "EFF_Device::Device_GetPropertyData: not enough space for the return value of kAudioDevicePropertyNominalSampleRate for the device");
        ...
        case kAudioDevicePropertyAvailableNominalSampleRates:
            ...
            //
            //  EFFDevice supports any sample rate so it can be set to match the output
            ...
            if(theNumberItemsToFetch > 0)
            {
                // 0 would cause divide-by-zero errors in other EFF_Device functions (and
        ...
        case kAudioDevicePropertyPreferredChannelsForStereo:
            ...
            ThrowIf(inDataSize < (2 * sizeof(UInt32)),
                    CAException(kAudioHardwareBadPropertySizeError),
                    "EFF_Device::Device_GetPropertyData: not enough space for the return value of kAudioDevicePropertyPreferredChannelsForStereo for the device");
            ((UInt32*)outData)[0] = 1;
            ((UInt32*)outData)[1] = 2;
        ...
        case kAudioDevicePropertyPreferredChannelLayout:
            ...
            //    by default. For this device, we return a stereo ACL.
            {
                ...
                ThrowIf(inDataSize < theACLSize,
                        CAException(kAudioHardwareBadPropertySizeError),
                        "EFF_Device::Device_GetPropertyData: not enough space for the return value of kAudioDevicePropertyPreferredChannelLayout for the device");
        ...
        case kAudioDevicePropertyZeroTimeStampPeriod:
            ...
            ThrowIf(inDataSize < sizeof(UInt32),
                    CAException(kAudioHardwareBadPropertySizeError),
                    "EFF_Device::Device_GetPropertyData: not enough space for the return value of kAudioDevicePropertyZeroTimeStampPeriod for the device");
        ...
        case kAudioDevicePropertyIcon:
            {
                ...
                ThrowIf(inDataSize < sizeof(CFURLRef),
                        CAException(kAudioHardwareBadPropertySizeError),
                        "EFF_Device::Device_GetPropertyData: not enough space for the return value of kAudioDevicePropertyIcon for the device");
                
                CFBundleRef theBundle = CFBundleGetBundleWithIdentifier(EFF_PlugIn::GetInstance().GetBundleID());
                ThrowIf(theBundle == NULL,
                        CAException(kAudioHardwareUnspecifiedError),
                        "EFF_Device::Device_GetPropertyData: could not get the plugin bundle for kAudioDevicePropertyIcon");
                
                CFURLRef theURL = CFBundleCopyResourceURL(theBundle, CFSTR("DeviceIcon.icns"), NULL, NULL);
                ThrowIf(theURL == NULL,
                        CAException(kAudioHardwareUnspecifiedError),
                        "EFF_Device::Device_GetPropertyData: could not get the URL for kAudioDevicePropertyIcon");
        ...
        case kAudioObjectPropertyCustomPropertyInfoList:
            ...
            if(theNumberItemsToFetch > 0)
            {
                ((AudioServerPlugInCustomPropertyInfo*)outData)[0].mSelector =
                    kAudioDeviceCustomPropertyAppVolumes;
                ((AudioServerPlugInCustomPropertyInfo*)outData)[0].mPropertyDataType =
                    kAudioServerPlugInCustomPropertyDataTypeCFPropertyList;
                ((AudioServerPlugInCustomPropertyInfo*)outData)[0].mQualifierDataType =
            ...
            if(theNumberItemsToFetch > 2)
            {
                ((AudioServerPlugInCustomPropertyInfo*)outData)[2].mSelector =
            ...
            {
                ((AudioServerPlugInCustomPropertyInfo*)outData)[3].mSelector = kAudioDeviceCustomPropertyDeviceIsRunningSomewhereOtherThanEFFApp;
            ...
            {
                ((AudioServerPlugInCustomPropertyInfo*)outData)[4].mSelector =
        ...
        case kAudioDeviceCustomPropertyDeviceAudibleState:
            {
                ThrowIf(inDataSize < sizeof(CFNumberRef),
                        CAException(kAudioHardwareBadPropertySizeError),
                        "EFF_Device::Device_GetPropertyData: not enough space for the return value of kAudioDeviceCustomPropertyDeviceAudibleState for the device");

                // The audible state is read without locking to avoid priority inversions on the IO threads.
                EFFDeviceAudibleState theAudibleState = mAudibleState.GetState();
        ...
        case kAudioDeviceCustomPropertyMusicPlayerProcessID:
            {
                ThrowIf(inDataSize < sizeof(CFNumberRef),
                        CAException(kAudioHardwareBadPropertySizeError),
                        "EFF_Device::Device_GetPropertyData: not enough space for the return value of kAudioDeviceCustomPropertyMusicPlayerProcessID for the device");
                CAMutex::Locker theStateLocker(mStateMutex);
        ...
        case kAudioDeviceCustomPropertyMusicPlayerBundleID:
            {
                ThrowIf(inDataSize < sizeof(CFStringRef),
                        CAException(kAudioHardwareBadPropertySizeError),
                        "EFF_Device::Device_GetPropertyData: not enough space for the return value of kAudioDeviceCustomPropertyMusicPlayerBundleID for the device");
                CAMutex::Locker theStateLocker(mStateMutex);
        ...
        case kAudioDeviceCustomPropertyDeviceIsRunningSomewhereOtherThanEFFApp:
            ThrowIf(inDataSize < sizeof(CFBooleanRef),
                    CAException(kAudioHardwareBadPropertySizeError),
                    "EFF_Device::Device_GetPropertyData: not enough space for the return value of kAudioDeviceCustomPropertyDeviceIsRunningSomewhereOtherThanEFFApp for the device");
            *reinterpret_cast<CFBooleanRef*>(outData) =
                mClients.ClientsOtherThanEFFAppRunningIO() ? kCFBooleanTrue : kCFBooleanFalse;
        ...
        case kAudioDeviceCustomPropertyAppVolumes:
            {
                ThrowIf(inDataSize < sizeof(CFArrayRef),
                        CAException(kAudioHardwareBadPropertySizeError),
                        "EFF_Device::Device_GetPropertyData: not enough space for the return value of kAudioDeviceCustomPropertyAppVolumes for the device");
                CAMutex::Locker theStateLocker(mStateMutex);
        ...
        case kAudioDeviceCustomPropertyEnabledOutputControls:
            {
                ThrowIf(inDataSize < sizeof(CFArrayRef),
                        CAException(kAudioHardwareBadPropertySizeError),
                        "EFF_Device::Device_GetPropertyData: not enough space for the return value of kAudioDeviceCustomPropertyEnabledOutputControls for the device");
...
void    EFF_Device::Device_SetPropertyData(AudioObjectID inObjectID,
                                           pid_t inClientPID,
                                           const AudioObjectPropertyAddress& inAddress,
                                           UInt32 inQualifierDataSize,
                                           const void* inQualifierData,
                                           ...
                                           const void* inData)
{
    switch(inAddress.mSelector)
    {
        case kAudioDevicePropertyNominalSampleRate:
            ThrowIf(inDataSize < sizeof(Float64),
                    CAException(kAudioHardwareBadPropertySizeError),
                    "EFF_Device::Device_SetPropertyData: wrong size for the data for kAudioDevicePropertyNominalSampleRate");
        ...
        case kAudioDeviceCustomPropertyMusicPlayerProcessID:
            {
                ThrowIf(inDataSize < sizeof(CFNumberRef),
                        CAException(kAudioHardwareBadPropertySizeError),
                        "EFF_Device::Device_SetPropertyData: wrong size for the data for kAudioDeviceCustomPropertyMusicPlayerProcessID");
                
                CFNumberRef pidRef = *reinterpret_cast<const CFNumberRef*>(inData);
                
                ThrowIf(pidRef == NULL,
                        CAException(kAudioHardwareIllegalOperationError),
                        "EFF_Device::Device_SetPropertyData: null reference given for kAudioDeviceCustomPropertyMusicPlayerProcessID");
                ThrowIf(CFGetTypeID(pidRef) != CFNumberGetTypeID(),
                        CAException(kAudioHardwareIllegalOperationError),
                        "EFF_Device::Device_SetPropertyData: CFType given for kAudioDeviceCustomPropertyMusicPlayerProcessID was not a CFNumber");
                ...
                ThrowIf(!success, CAException(kAudioHardwareIllegalOperationError),
                        "EFF_Device::Device_SetPropertyData: probable error from CFNumberGetValue when reading pid for kAudioDeviceCustomPropertyMusicPlayerProcessID");
                ...
                catch(EFF_InvalidClientPIDException)
                {
                ...
                if(propertyWasChanged)
                {
                    ...
                    CADispatchQueue::GetGlobalSerialQueue().Dispatch(false,    ^{
                        AudioObjectPropertyAddress theChangedProperties[] = {
                            kEFFMusicPlayerProcessIDAddress,
                            kEFFMusicPlayerBundleIDAddress
                        };
                        EFF_PlugIn::Host_PropertiesChanged(inObjectID, 2, theChangedProperties);
                    });
                }
            }
            break;
        ...
        case kAudioDeviceCustomPropertyMusicPlayerBundleID:
            {
                ThrowIf(inDataSize < sizeof(CFStringRef),
                        CAException(kAudioHardwareBadPropertySizeError),
                        "EFF_Device::Device_SetPropertyData: wrong size for the data for kAudioDeviceCustomPropertyMusicPlayerBundleID");
            
                CFStringRef theBundleIDRef = *reinterpret_cast<const CFStringRef*>(inData);
                
                ThrowIfNULL(theBundleIDRef,
                            CAException(kAudioHardwareIllegalOperationError),
                            "EFF_Device::Device_SetPropertyData: kAudioDeviceCustomPropertyMusicPlayerBundleID cannot be set to NULL");
                ThrowIf(CFGetTypeID(theBundleIDRef) != CFStringGetTypeID(),
                        CAException(kAudioHardwareIllegalOperationError),
                        "EFF_Device::Device_SetPropertyData: CFType given for kAudioDeviceCustomPropertyMusicPlayerBundleID was not a CFString");
                ...
                // we don't throw custom exceptions in SetMusicPlayer(const CACFString)
                // It was handled with an assert
        ...
        case kAudioDeviceCustomPropertyAppVolumes:
            {
                ThrowIf(inDataSize < sizeof(CFArrayRef),
                        CAException(kAudioHardwareBadPropertySizeError),
                        "EFF_Device::Device_SetPropertyData: wrong size for the data for kAudioDeviceCustomPropertyAppVolumes");
                
                CFArrayRef arrayRef = *reinterpret_cast<const CFArrayRef*>(inData);

                ThrowIfNULL(arrayRef,
                            CAException(kAudioHardwareIllegalOperationError),
                            "EFF_Device::Device_SetPropertyData: kAudioDeviceCustomPropertyAppVolumes cannot be set to NULL");
                ThrowIf(CFGetTypeID(arrayRef) != CFArrayGetTypeID(),
                        CAException(kAudioHardwareIllegalOperationError),
                        "EFF_Device::Device_SetPropertyData: CFType given for kAudioDeviceCustomPropertyAppVolumes was not a CFArray");
                ...
                catch(EFF_InvalidClientRelativeVolumeException)
                {
                    Throw(CAException(kAudioHardwareIllegalOperationError));
                }
                ...
                if(propertyWasChanged)
                {
                    // Send notification
                    CADispatchQueue::GetGlobalSerialQueue().Dispatch(false,    ^{
                        AudioObjectPropertyAddress theChangedProperties[] = { kEFFAppVolumesAddress };
                        EFF_PlugIn::Host_PropertiesChanged(inObjectID, 1, theChangedProperties);
                    });
                }
            }
            break;
        ...
        case kAudioDeviceCustomPropertyEnabledOutputControls:
            {
                ThrowIf(inDataSize < sizeof(CFArrayRef),
                        CAException(kAudioHardwareBadPropertySizeError),
                        "EFF_Device::Device_SetPropertyData: wrong size for the data for "
                ...
                ThrowIfNULL(theEnabledControlsRef,
                            CAException(kAudioHardwareIllegalOperationError),
                            "EFF_Device::Device_SetPropertyData: null reference given for "
                            "kAudioDeviceCustomPropertyEnabledOutputControls");
                ThrowIf(CFGetTypeID(theEnabledControlsRef) != CFArrayGetTypeID(),
                        CAException(kAudioHardwareIllegalOperationError),
                        "EFF_Device::Device_SetPropertyData: CFType given for "
                ...
                ThrowIf(theEnabledControls.GetNumberItems() != 2,
                        CAException(kAudioHardwareIllegalOperationError),
                        "EFF_Device::Device_SetPropertyData: Expected the CFArray given for "
                        "kAudioDeviceCustomPropertyEnabledOutputControls to have exactly 2 elements");

                bool theVolumeControlEnabled;
                bool didGetBool = theEnabledControls.GetBool(kEFFEnabledOutputControlsIndex_Volume,
                                                             theVolumeControlEnabled);
                ThrowIf(!didGetBool,
                        CAException(kAudioHardwareIllegalOperationError),
                        "EFF_Device::Device_SetPropertyData: Expected CFBoolean for volume elem of "
                        "kAudioDeviceCustomPropertyEnabledOutputControls");

                bool theMuteControlEnabled;
                didGetBool = theEnabledControls.GetBool(kEFFEnabledOutputControlsIndex_Mute,
                                                        theMuteControlEnabled);
                ThrowIf(!didGetBool,
                        CAException(kAudioHardwareIllegalOperationError),
                        "EFF_Device::Device_SetPropertyData: Expected CFBoolean for mute elem of "
                        "kAudioDeviceCustomPropertyEnabledOutputControls");
...
{
    bool clientIsEFFApp;
    bool EFFAppHasClientRegistered;

    {
        ...
        //   - A client starts IO.
        //   - The plugin host (the HAL) calls the StartIO function in EFF_PlugInInterface, which calls this function.
        //   - EFFDriver sends a message to EFFApp telling it to start the (real) audio hardware.
        //   - EFFApp starts the hardware and, after the hardware is ready, replies to EFFDriver's message.
        //   - EFFDriver lets the host know that it's ready to do IO by ret///////////////////'urning from StartIO.
        ...
        //
        // didStartIO ultimately gets returned from EFF_Clients::StartIONonRT. It will be true if
        // no other clients were running IO before this one started
        ...
        if(didStartIO)
        {
            ...
            ThrowIfKernelError(theError,
                               ...
                               "EFF_Device::StartIO: Failed to start because of an error calling down to the driver.");
        }

        clientIsEFFApp = mClients.IsEFFApp(inClientID);
        EFFAppHasClientRegistered = mClients.EFFAppHasClientRegistered();
    }
    ...
    if(!clientIsEFFApp && EFFAppHasClientRegistered)
    {
        ...
        switch(theXPCError)
        {
            case kEFFXPC_Success:
                DebugMsg("EFF_Device::StartIO: Ready for IO.");
                break;
        
            case kEFFXPC_MessageFailure:
                // This most likely means EFFXPCHelper isn't installed or has crashed. IO will probably still work,
                // but we may drop frames while the audio hardware starts up.
                LogError("EFF_Device::StartIO: Couldn't reach EFFApp via XPC. Attempting to start IO anyway.");
                break;
                
            case kEFFXPC_ReturningEarlyError:
                // This can (and might always) happen when the user changes output device in EFFApp while IO is running.
                // See EFFAudioDeviceManager::startPlayThroughSync and EFFPlayThrough::WaitForOutputDeviceToStart.
                LogWarning("EFF_Device::StartIO: EFFApp was busy, so EFFDriver has to return from StartIO early.");
                break;
                
            default:
                LogError("EFF_Device::StartIO: EFFApp failed to start the output device. theXPCError=%llu", theXPCError);
...
void    EFF_Device::StopIO(UInt32 inClientID)
{
    ...
    //
    // The task ultimately calls EFF_Clients::StopIONonRT
...
void    EFF_Device::GetZeroTimeStamp(Float64& outSampleTime,
                                     UInt64& outHostTime,
                                     UInt64& outSeed)
{
    ...
    else
    {
        ...
        // The offset is the # of ticks from the anchor time to the next sample time
        // Consequently the next host time is the host time at the next sample time
        ...
        // advance to the next sample time if the calculated next host time is less than the current host time
        ...
        outHostTime = static_cast<UInt64>(mLoopbackTime.anchorHostTime +
...
void    EFF_Device::WillDoIOOperation(UInt32 inOperationID,
                                      bool& outWillDo,
                                      bool& outWillDoInPlace)
const
{
...
void    EFF_Device::BeginIOOperation(UInt32 inOperationID,
                                     ...
                                     UInt32 inClientID)
{
    ...
    if(inOperationID == kAudioServerPlugInIOOperationThread)
    {
        ...
        // kAudioDeviceCustomPropertyDeviceIsRunningSomewhereOtherThanEFFApp. We have to do this here
...
void    EFF_Device::DoIOOperation(AudioObjectID inStreamObjectID,
                                  UInt32 inClientID,
                                  ...
                                  void* ioSecondaryBuffer)
{
    ...
    switch(inOperationID)
    {
        ...
        case kAudioServerPlugInIOOperationProcessOutput:
            // From docs: This operation is about the buffer for one particular client.
            {
        ...
        case kAudioServerPlugInIOOperationProcessMix:
            // From docs: This operation processes the full mix of all clients' data in the canonical format.
            {
                // Check the arguments.
                ThrowIfNULL(ioMainBuffer,
                            CAException(kAudioHardwareIllegalOperationError),
                            "EFF_Device::DoIOOperation: Buffer for "
        ...
        case kAudioServerPlugInIOOperationWriteMix:
            // TODO: don't know but maybe this is where we can record things
            {
                CAMutex::Locker theIOLocker(mIOMutex);

                bool didChangeState = mAudibleState.UpdateWithMixedIO(inIOBufferFrameSize,
                                                                      inIOCycleInfo.mOutputTime.mSampleTime,
                                                                      reinterpret_cast<const Float32*>(ioMainBuffer));
        ...
        default:
            ...
            DebugMsg("EFF_Device::DoIOOperation: Unexpected IO operation: %u", inOperationID);
            break;
    };
}

void    EFF_Device::EndIOOperation(UInt32 inOperationID,
                                   UInt32 inIOBufferFrameSize,
                                   const AudioServerPlugInIOCycleInfo& inIOCycleInfo,
                                   UInt32 inClientID)
{
    ...
    if(inOperationID == kAudioServerPlugInIOOperationThread)
    {
        // Tell EFF_Clients that this client has stopped IO. Queued async because we have to be real-time safe here.
        //
...
void    EFF_Device::ReadInputData(UInt32 inIOBufferFrameSize, Float64 inSampleTime, void* outBuffer)
{
    ...
    AudioBufferList abl = {
        ...
        .mBuffers[0] = {
            .mNumberChannels = 2,
            // Each frame is 2 Float32 samples (one per channel).
            // # of frames * # of bytes per frame = size of outBuffer in bytes
    ...
    CARingBufferError err = mLoopbackRingBuffer.Fetch(&abl,
                                                      inIOBufferFrameSize,
...
void    EFF_Device::WriteOutputData(UInt32 inIOBufferFrameSize,
                                    ...
                                    const void* inBuffer)
{
    ...
    CARingBufferError err = mLoopbackRingBuffer.Store(&abl,
                                                      inIOBufferFrameSize,
                                                      static_cast<CARingBuffer::SampleTime>(inSampleTime));
...
void    EFF_Device::ApplyClientRelativeVolume(UInt32 inClientID,
                                              UInt32 inIOBufferFrameSize,
                                              void* ioBuffer)
const
{
...
void    EFF_Device::RequestEnabledControls(bool inVolumeEnabled, bool inMuteEnabled)
{
    CAMutex::Locker theStateLocker(mStateMutex);
    ...
    if(changeVolume)
    {
        DebugMsg("EFF_Device::RequestEnabledControls: %s volume control",
    ...
    if(changeMute)
    {
        DebugMsg("EFF_Device::RequestEnabledControls: %s mute control",
    ...
    if(changeVolume || changeMute)
    {
        ...
        CADispatchQueue::GetGlobalSerialQueue().Dispatch(false,    ^{
            EFF_PlugIn::Host_RequestDeviceConfigurationChange(theDeviceObjectID, action, nullptr);
        });
    }
}

Float64    EFF_Device::GetSampleRate()
const
{
...
void    EFF_Device::RequestSampleRate(Float64 inRequestedSampleRate)
{
    ...
    ThrowIf(inRequestedSampleRate < 1.0,
            ...
            "EFF_Device::RequestSampleRate: unsupported sample rate");

    DebugMsg("EFF_Device::RequestSampleRate: Sample rate change requested: %f",
    ...
    if(inRequestedSampleRate != GetSampleRate())  // Check the sample rate will actually be changed.
    {
        ...
        auto requestSampleRate = ^{
            ...
            EFF_PlugIn::Host_RequestDeviceConfigurationChange(GetObjectID(), action, nullptr);
        };
        CADispatchQueue::GetGlobalSerialQueue().Dispatch(false, requestSampleRate);
    }
}

EFF_Object&  EFF_Device::GetOwnedObjectByID(AudioObjectID inObjectID)
{
    // C++ is weird. See "Avoid Duplication in const and Non-const Member Functions" in Item 3 of Effective C++.
    return const_cast<EFF_Object&>(static_cast<const EFF_Device&>(*this).GetOwnedObjectByID(inObjectID));
}

const EFF_Object&  EFF_Device::GetOwnedObjectByID(AudioObjectID inObjectID)
const
{
    ...
    {
        LogError("EFF_Device::GetOwnedObjectByID: Unknown object ID. inObjectID = %u", inObjectID);
        Throw(CAException(kAudioHardwareBadObjectError));
    }
}

UInt32    EFF_Device::GetNumberOfSubObjects()
const
{
    return kNumberOfInputSubObjects + GetNumberOfOutputSubObjects();
}

UInt32    EFF_Device::GetNumberOfOutputSubObjects()
const
{
    return kNumberOfOutputStreams + GetNumberOfOutputControls();
}

UInt32    EFF_Device::GetNumberOfOutputControls()
const
{
    CAMutex::Locker theStateLocker(mStateMutex);
...
void    EFF_Device::SetEnabledControls(bool inVolumeEnabled, bool inMuteEnabled)
{
    CAMutex::Locker theStateLocker(mStateMutex);

    if(mVolumeControl.IsActive() != inVolumeEnabled)
    {
        DebugMsg("EFF_Device::SetEnabledControls: %s the volume control",
                 inVolumeEnabled ? "Enabling" : "Disabling");
    ...
    {
        DebugMsg("EFF_Device::SetEnabledControls: %s the mute control",
                 inMuteEnabled ? "Enabling" : "Disabling");
...
void EFF_Device::SetSampleRate(Float64 inSampleRate, bool force)
{
    // We try to support any sample rate a real output device might.
    ThrowIf(inSampleRate < 1.0,
            CAException(kAudioDeviceUnsupportedFormatError),
            "EFF_Device::SetSampleRate: unsupported sample rate");
    ...
    {
        DebugMsg("EFF_Device::SetSampleRate: Changing the sample rate from %f to %f",
        ...
        if(mWrappedAudioEngine != nullptr)
        {
            ...
            ThrowIfKernelError(theError,
                               CAException(kAudioHardwareUnspecifiedError),
                               "EFF_Device::SetSampleRate: Error setting the sample rate on the "
    ...
    {
        DebugMsg("EFF_Device::SetSampleRate: The sample rate is already set to %f", inSampleRate);
    }
}

bool    EFF_Device::IsStreamID(AudioObjectID inObjectID)
const noexcept
{
...
void    EFF_Device::_HW_Open()
{
}

void    EFF_Device::_HW_Close()
{
}

kern_return_t    EFF_Device::_HW_StartIO()
{
    EFFAssert(mStateMutex.IsOwnedByCurrentThread(),
              "EFF_Device::_HW_StartIO: Called without taking the state mutex");
    ...
    // at a time).
    EFFAssert(mIOMutex.IsFree(), "EFF_Device::_HW_StartIO: IO mutex taken before starting IO");
    mAudibleState.Reset();
    
    return KERN_SUCCESS;
}

void    EFF_Device::_HW_StopIO()
{
    if(mWrappedAudioEngine != NULL)
    {
    }
...
Float64    EFF_Device::_HW_GetSampleRate()
const
{
    // This function should only be called when wrapping a device.
    ThrowIf(mWrappedAudioEngine == nullptr,
            CAException(kAudioHardwareUnspecifiedError),
            "EFF_Device::_HW_GetSampleRate: No wrapped audio device");

    return mWrappedAudioEngine->GetSampleRate();
}

kern_return_t    EFF_Device::_HW_SetSampleRate(Float64 inNewSampleRate)
{
    // This function should only be called when wrapping a device.
    ThrowIf(mWrappedAudioEngine == nullptr,
            CAException(kAudioHardwareUnspecifiedError),
            "EFF_Device::_HW_SetSampleRate: No wrapped audio device");

    return mWrappedAudioEngine->SetSampleRate(inNewSampleRate);
}


UInt32    EFF_Device::_HW_GetRingBufferFrameSize()
const
{
...
void    EFF_Device::AddClient(const AudioServerPlugInClientInfo* inClientInfo)
{
    DebugMsg("EFF_Device::AddClient: Adding client %u (%s)",
...
void    EFF_Device::RemoveClient(const AudioServerPlugInClientInfo* inClientInfo)
{
    DebugMsg("EFF_Device::RemoveClient: Removing client %u (%s)",
             inClientInfo->mClientID,
             CFStringGetCStringPtr(inClientInfo->mBundleID, kCFStringEncodingUTF8));
    
    CAMutex::Locker theStateLocker(mStateMutex);

    // If we're removing EFFApp, reenable all of EFFDevice's controls.
    if(mClients.IsEFFApp(inClientInfo->mClientID))
    {
...
void    EFF_Device::PerformConfigChange(UInt64 inChangeAction, void* inChangeInfo)
{
    #pragma unused(inChangeInfo)
    DebugMsg("EFF_Device::PerformConfigChange: inChangeAction = %llu", inChangeAction);
...
void    EFF_Device::AbortConfigChange(UInt64 inChangeAction, void* inChangeInfo)
{
